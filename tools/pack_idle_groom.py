"""Pack idle_groom: sit → lift → lick → wipe_cheek → wipe_ear → lick → lift → sit.

Usage:
  python tools/pack_idle_groom.py
"""
...
SESSION = Path(
    ...
    r"\01a013dc-b530-7962-8f2d-16d0356fffdf\images"
)
OUT = PET / "idle_groom"
QA = ROOT / "target" / "_groom_pack"
SIZE = 256
FPS = 50.0
...
SOURCES = {
    "lift": SESSION / "3.jpg",
    "lick": SESSION / "4.jpg",
    "wipe_cheek": SESSION / "5.jpg",
    "wipe_ear": SESSION / "6.jpg",
}
...
def main() -> None:
    ...
    slugs = {
        "lift": "01_lift",
        "lick": "02_lick",
        "wipe_cheek": "03_wipe_cheek",
        "wipe_ear": "04_wipe_ear",
    }
    for name, src in SOURCES.items():
        if not src.is_file():
            raise SystemExit(f"missing {src}")
        dest = MASTER / f"groom_{slugs[name]}.jpg"
        shutil.copy2(src, dest)
        raw = downscale_premul(soft_key_magenta(load_rgb(src)))
        locked = foot_lock(raw, sit)
        save_rgba(locked, MASTER / f"groom_{slugs[name]}_keyed.png")
    ...
    lift = keyed["lift"]
    lick = keyed["lick"]
    wipe_cheek = keyed["wipe_cheek"]
    wipe_ear = keyed["wipe_ear"]
    ...
    frames = (
        ...
        + hold(lift, HOLD_LIFT)
        + hold(lick, HOLD_LICK)
        + hold(wipe_cheek, HOLD_WIPE_CHEEK)
        + hold(wipe_ear, HOLD_WIPE_EAR)
        + hold(lick, HOLD_LICK2)
        + hold(lift, HOLD_LIFT_OUT)
        + hold(sit, HOLD_SIT_OUT)
    )
    if len(frames) != expected:
        raise SystemExit(f"frame count {len(frames)} != {expected}")
    ...
    peak_start = HOLD_SIT_IN + HOLD_LIFT
    peak_end = peak_start + HOLD_LICK - 1
    meta = {
        "name": "idle_groom",
        "frame_width": SIZE,
        "frame_height": SIZE,
        ...
        "peak_end": peak_end,
        "source": "master_groom_keys",
        "notes": (
            "Sit bookend exact. 50fps holds. Lift/lick/wipe/wipe/lick/lift. "
            "No reverse of whole clip. No sit-face stamp. No flow."
        ),
    }
    (OUT / "meta.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")

    keys = [sit, lift, lick, wipe_cheek, wipe_ear]
