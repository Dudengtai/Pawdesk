...
| 项目 | 内容 |
| --- | --- |
| 版本 | **v0.18**（2026-08-17：启动坞「最近启用」+「应用列表」分区） |
| 依据 | `prd.md` v0.7.5 · `tech.md` v0.15 |
...
| 列表 | 标题「应用列表」+ 常用应用行（图标圆点 + 名 + chevron）；**固定视口 + 滚轮** |
...
- 打开方式：从启动坞右上角肉垫印或失效行点击时，**窗与猫钉死**，设置在原坞卡内从右侧滑入（220ms ease-in-out）；「完成」反转滑回启动坞。托盘「打开设置」保持居中直开。
- 与启动坞同一套玻璃 / 圆角 / 字号气质。  
- 分区顺序：  
  1. **健康提醒**（开关、间隔、暂停）  
  2. **宠物大小**（`−` / 百分比 / `+`，相对设计基准）  
