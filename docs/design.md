...
- 提醒窗口 **560×420**：整图 `reminder_card.png`（猫举杯 + 漫画气泡两行文案）contain-fit 显示，不裁切、不叠框；杯子完整可见。  
- 文案两行：「你这是要把自己风干成木乃伊吗？」/「快给内脏浇浇水」。  
- 底部中央满猫粮碗（`assets/ui/feed_bowl.png`）当投喂按钮。
...
- 关闭设置 ≠ 退出应用。  
- 调大小时：卡片屏幕位置与叠层窗几何**钉死**；宠按体型贴着卡片保持 `DEFAULT_GAP`（超出画布的部分裁掉，不扩窗）。「完成」提交尺寸并把待机原点钉在预览位置；Esc 丢预览、几何回进入时。托盘变大/变小与设置同一上下限，待机即时改窗。
...
| 改宠物大小 | 待机即时改窗；设置内卡片钉死、宠贴间距预览，「完成」停在新位置 | — |
