...
| 项目 | 内容 |
| --- | --- |
| 版本 | **v0.27**（2026-08-18：提醒进场/回程叠层内轻跃） |
| 依据 | `prd.md` v0.7.12 · `tech.md` v0.25 |
...
| 撒娇 | Idle one-shot | **`idle_yawn`**（峰值「困死我了…」）、**`idle_stretch`**（紧凑左侧伸；峰值开心眯眼吐舌；无气泡）或 **`idle_groom`**（正面坐、左爪舔再擦脸；无气泡） |
| 在看你 | Watching | 仍坐姿待机；头眼跟光标；不左右镜像 |
| 被拎起 | Dragging | 母版坐姿 + 轻微 scale 脉冲 |
| 躲边 | HiddenAtEdge | 母版坐姿半隐藏 |
| 催你起来 | Reminder | 叠层内轻跃到中央 → 卡片（tishi）→ 投喂 → 叠层内轻跃回原位 |
...
- 约 **60s** 打一次哈欠、伸懒腰或舔爪洗脸（轮播）；鼠标在旁边观察时不要饿死计时。  
- 调度：`IDLE_ACTION_ENABLED = ["idle_yawn", "idle_stretch", "idle_groom"]`。旧 cute/sleep/wag/watch **已下盘**。  
- 哈欠：同一只正面猫；峰值奶油色墨线气泡「困死我了…」，默认出在头右侧。拖动 / 开坞可打断并回坐。  
- 伸懒腰：同一只猫紧凑左侧伸（转 → 下蹲 → 探 → 半峰 → 峰）；墨线与毛发质感跟母版；峰值开心眯眼吐舌。无气泡、不拓窗；进出与待机同一 letterbox。  
- 舔爪洗脸：同一只坐猫；抬爪仍正面，舔/擦时头肩微侧（不趴下、不换猫）。只抬一只前爪。分镜 sit → 抬爪 → 舔爪 → 擦颊 → 擦耳 → 再舔 → 回落 → sit（非对称，不倒放整段）。英雄键闭眼满足、小粉舌舔爪垫；擦脸时舌收回。无气泡、不拓窗；进出与待机同一 letterbox。  
...
- 打包舔爪洗脸：`python tools/pack_idle_groom.py`（母版 4 键 + sit 书挡 hold；品红软抠；书挡 ≡ `idle_blink/000`）。  
...
```text
原位坐姿 → 一次长大叠层 → 攒劲下蹲 → 叠层内抛物线轻跃 → 落地回坐
  → 同叠层 fade 出提醒卡片（举杯 / 小喇叭轮换）→ 点击投喂
  → 卡 fade 出 → 叠层内轻跃回原位 → 一次缩回待机
```

- 旅途 / 回程 **不搬小 HWND、不播 `reminder_hop`**。HWND 原点与尺寸只设一次；猫在缓冲里走（与哈欠 / 启动坞同一纪律）。朝向始终正面，不镜像。  
- 进度：0–16% 原地攒劲（squash）→ 16–84% ease-in-out 抛物线 → 84–100% 钉在终点回坐。位移 100% 来自槽插值；精灵保持母版坐姿 letterbox。  
- 时长随距离 480–800ms；已在中央附近（&lt;56px）不跳，叠层仍一次长大后直接出卡。躲边先瞬间露全身再跳。  
- 落地后 140ms 卡 fade 入；投喂后 120ms 卡 fade 出再回跳。到位 / 投喂不叠活宠（卡里已有猫）。

- 提醒窗口 **640×360（16:9）**：整图卡片 contain-fit 铺满窗口，不裁切、不叠框。两张轮换（进场时抽一张，本轮不中途换图，避免连抽同一张）：  
  - `reminder_card.png`：猫举杯。「你这是要把自己风干成木乃伊吗？」/「快给内脏浇浇水」。杯子完整可见。  
  - `reminder_card_activity.png`：猫拿小喇叭喊。「喂我之前，」/「先站起来活动活动吧~」。喇叭完整可见。  
- 猫粮碗（`assets/ui/feed_bowl.png`，96×96）落在气泡正下方的透明空档（左下），不挡猫、不当正中底边孤岛；两张卡共用同一碗位。
...
（整图卡片采用图中固定文案并在卡片之间轮换；代码文案池仅作缺图 fallback 保留。）
...
| 项 | 说明 |
| --- | --- |
| 图标 | 宠物头像感小图；**Release 下这是任务栏旁唯一的程序身份**（无控制台、宠窗不进任务栏 / Alt-Tab） |
...
| 提醒进场 / 回程 | 叠层内轻跃：HWND 钉死 + sit squash/stretch + 槽抛物线 | in-out cubic + hop arc 480–800ms |
...
| **v0.26** | **2026-08-18** | 舔爪洗脸 `idle_groom` 进 60s 轮播；正面坐、左爪舔擦、无气泡不拓窗；prd **v0.7.11** · tech **v0.24** |
| **v0.27** | **2026-08-18** | 提醒进场/回程改为叠层内轻跃（HWND 钉死、sit+squash、同缓冲出卡）；不再搬小窗播 `reminder_hop`；prd **v0.7.12** · tech **v0.25** |
