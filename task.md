...
| 文档类型 | 开发任务与模块排期 |
| 当前版本 | **v0.3**（进度同步 2026-08-04，下阶段规划） |
| 依据文档 | `prd.md` v0.2、`tech.md` v0.1、`design.md` v0.1 |
| 环境参考 | `env.md` |
| 状态 | **M0–M5 功能可日常使用**；**下阶段 = 改 bug → 交互逻辑 → UI 修复**；**宠物动作/形象最后** |
...
| **M6** | — | — | **待开** | **改 bug → 交互逻辑 → UI 修复**；宠物动作/形象**最后** |

**当前建议下一动作（严格优先级）**：见 **§13 下阶段计划**。
...
| ID | 期望 | 现状 |
| --- | --- | --- |
| IX-01 | 启动坞开/关锚定稳、少跳位 | 基本可用，边角场景可再验 |
...
| **M6 质量迭代** | ⬜ **下一阶段** | bug → 交互 → UI；**形象/动作最后** |
...
2. 交互逻辑符合预期、可预测  
3. UI 手感与对齐问题收敛  
4. **明确不在本阶段做**：大形象重做、新动作流水线、飞扑重开（列入 M6-C / 更后）
...
| 子阶段 | 内容 | 完成标志 |
| --- | --- | --- |
| **M6-A** | 修 §11.2-A；回归隐藏/提醒/异步添加/开关设置 | 无必现崩溃；主路径手工清单过一遍 |
| **M6-B** | §11.2-B：锚定、hover 态、优先级验收、阈值微调 | 点击/拖动/菜单/提醒不再「抢状态」 |
| **M6-C** | §11.2-C：启动坞/设置/提醒 UI 一致性 | 观感明显优于 M5，仍可不碰精灵资源 |
| **M6-D** | §11.2-D：动作 crossfade、飞扑、形象 | **仅当 A–C 收口后**，单独开迭代 |
...
- 每修一批 bug：在 §11.2 勾掉或改状态  
- UI 改动：截图对比 + 不破坏 DPR 1:1  
